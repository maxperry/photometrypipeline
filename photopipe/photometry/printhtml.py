...
def plot_seds_test():
    ...
    plot_seds(filters, colors)

def plot_seds_1(filters, colors):
    wavelengths = { 'r': 6122.3, 'i': 7439.5, 'z': 8897.1, 'y': 10289.4, 'J': 12350.0, 'K': 21590.0, 'u': 3594.9, 'g': 4640.4, 'H':16620.0 }
    file_mask = ['index', 'RA', 'DEC', 'u', 'g', 'r', 'i', 'z', 'y', 'B', 'V', 'R', 'I', 'J', 'H', 'K', 'u_err', 'g_err', 'r_err', 'i_err', 'z_err', 'y_err', 'B_err', 'V_err', 'R_err', 'I_err', 'J_err', 'H_err', 'K_err', 'Mode']
    ...
    for file in files:
        ...
        for i in range(len(data)):
            source_index = str(int(data[i][0]))
            filter_sources[source_index] = np.array([data[i]]) if not (source_index in filter_sources) else np.append(filter_sources[source_index], np.array([data[i]]), 0)

        for source_index, seds in filter_sources.iteritems():
            pl.figure()

            for i, sed in enumerate(seds): # Assuming len = 1
                print 'source: ' + source_index + ' sed: ' + str(i)
                plot_x = []
                plot_y = []
                ...
                print plot_y
                pl.plot(plot_x, plot_y, label=filter + '_' + str(i), color=colors[color_counter])

            pl.xlabel(r'Wavelength ($\AA$)')
            pl.ylabel('Magnitude')
            pl.legend(loc='lower right')
            pl.savefig('seds/' + source_index + '_' + filter + '.plot.png', bbox_inches='tight')
            pl.clf()

        color_counter = color_counter + 1

def plot_seds_no_index(filters, colors):
    wavelengths = { 'r': 6122.3, 'i': 7439.5, 'z': 8897.1, 'y': 10289.4, 'J': 12350.0, 'K': 21590.0, 'u': 3594.9, 'g': 4640.4, 'H':16620.0 }
    file_mask = ['RA', 'DEC', 'u', 'g', 'r', 'i', 'z', 'y', 'B', 'V', 'R', 'I', 'J', 'H', 'K', 'u_err', 'g_err', 'r_err', 'i_err', 'z_err', 'y_err', 'B_err', 'V_err', 'R_err', 'I_err', 'J_err', 'H_err', 'K_err', 'Mode', 'Timestamp']
    ...
    for file in files:
        ...
        for i in range(len(data)):
            source_index = str(data[i][0]) + '_' + str(data[i][1])
            filter_sources[source_index] = np.array([data[i]]) if not (source_index in filter_sources) else np.append(filter_sources[source_index], np.array([data[i]]), 0)
        ...
        for source_index, seds in filter_sources.iteritems():
            ...
            for i, sed in enumerate(seds): # Assuming len = 1
                ...
                for f in filters:
                    ...
                    plot_y.append(sed[file_mask.index(f)])
                print plot_y
                ax.plot(plot_x, plot_y, label=sed[file_mask.index('Timestamp')])

            pl.title(source_index.replace('_', ', ') + ' (' + filter + ')')
            pl.xlabel(r'Wavelength ($\AA$)')
            pl.ylabel('Magnitude')
...
def plot_seds(filters):
    ...
    for file in files:
        ...
        pl.subplots_adjust(top=0.8, hspace=0.3, wspace=0.3)
